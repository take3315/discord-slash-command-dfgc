...
async function selectMonth(interaction) {
    ...
    const filter = (i) => i.customId === 'monthselect' && i.user.id === interaction.user.id;
    const monthCollector = interaction.channel.createMessageComponentCollector({ filter, time: 15000 });
...
async function selectActivityLevel(interaction, selectedMonth) {
    ...
    const filter = (i) => i.customId === 'activityselect' && i.user.id === interaction.user.id;
    const activityCollector = interaction.channel.createMessageComponentCollector({ filter, time: 15000 });
...
async function selectToken(interaction) {
    ...
    // Wait for the user to select a month from the menu
    const filter = (i) => i.customId === 'tokenselect' && i.user.id === interaction.user.id;
    const tokenCollector = interaction.channel.createMessageComponentCollector({ filter, time: 15000 });
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === 'report') {
        ...
        const selectedActivityLevel = await selectActivityLevel(interaction, selectedMonth);
